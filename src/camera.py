...
class Camera:
    def __init__(self, center_obj, zoom):
    ...
    def update(self, full_map):
        ...
        if self.camera_pos_x[0] > self.center_obj.x and self.camera_pos_y[0] > self.center_obj.y:
            #print("1")
            self.x = self.camera_pos_x[0] * self.zoom
            self.y = self.camera_pos_y[0] * self.zoom

            self.center_obj.rect.center = [(self.center_obj.x + self.speed) * self.zoom, (self.center_obj.y + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)]

        # BOTTOM RIGHT
        elif self.camera_pos_x[1] < self.center_obj.x and self.camera_pos_y[1] < self.center_obj.y:
            #print(f"X: {(self.camera_pos_x[1] - (WIDTH / (2 * self.zoom)))}")
            #print(f"Y: {(self.camera_pos_y[1] - (HEIGHT / (2 * self.zoom)))}")
            #print("2")
            self.x = self.camera_pos_x[1] * self.zoom
            self.y = self.camera_pos_y[1] * self.zoom
            
            self.center_obj.rect.center = [(self.center_obj.x - (self.camera_pos_x[1] - (WIDTH / (2 * self.zoom))) + self.speed) * self.zoom, (self.center_obj.y - (self.camera_pos_y[1] - (HEIGHT / (2 * self.zoom))) + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)]
        ...
        # UPPER RIGHT
            ...
            self.center_obj.rect.center = [(self.center_obj.x - (self.camera_pos_x[1] - (WIDTH / (2 * self.zoom))) + self.speed) * self.zoom, (self.center_obj.y + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)]
        ...
        # BOTTOM LEFT
            ...
            self.center_obj.rect.center = [(self.center_obj.x + self.speed) * self.zoom, (self.center_obj.y - (self.camera_pos_y[1] - (HEIGHT / (2 * self.zoom))) + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)] 
        ...
        # STRAIGHT RIGHT
            ...
            #print(f"5")
            self.x = self.camera_pos_x[1] * self.zoom
            self.y = self.center_obj.y * self.zoom
            
            self.center_obj.rect.center = [(self.center_obj.x - (self.camera_pos_x[1] - (WIDTH / (2 * self.zoom))) + self.speed) * self.zoom, (self.camera_pos_y[0] + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - self.center_obj.rect.center[1]]

        # STRAIGHT BOTTOM
        elif self.camera_pos_y[1] < self.center_obj.y:
            #print(f"6")
            self.x = self.center_obj.x * self.zoom
            self.y = self.camera_pos_y[1] * self.zoom
            
            self.center_obj.rect.center = [(self.camera_pos_x[0] + self.speed) * self.zoom, (self.center_obj.y - (self.camera_pos_y[1] - (HEIGHT / (2 * self.zoom))) + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - self.center_obj.rect.center[0], self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)]        
        ...
        # STRAIGHT LEFT        
            ...
            self.center_obj.rect.center = [(self.center_obj.x + self.speed) * self.zoom, (self.camera_pos_y[0] + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - (self.gimble_effect[0] * self.zoom), self.y - self.anchor[1] - self.center_obj.rect.center[1]] 
        ...
        # STRAIGHT UP
            ...
            self.center_obj.rect.center = [(self.camera_pos_x[0] + self.speed) * self.zoom, (self.center_obj.y + self.speed) * self.zoom]
            self.gimble = [self.x - self.anchor[0] - self.center_obj.rect.center[0], self.y - self.anchor[1] - (self.gimble_effect[1] * self.zoom)] 
            ...
            self.center_obj.rect.center = [(self.camera_pos_x[0] + self.speed) * self.zoom, (self.camera_pos_y[0] + self.speed) * self.zoom]
    ...
    def addE(self, obj, screen):
        zoom_factor = self.zoom / TILE_SIZE
        image = pygame.image.load(obj.image_name)
        obj.image = pygame.transform.scale_by(image, zoom_factor)

        obj.rect.center = [((obj.x * self.zoom) - self.anchor[0] - self.gimble[0]), ((obj.y * self.zoom) - self.anchor[1] - self.gimble[1])]
    ...
    def addObj(self, obj, screen):
        ...
        if obj.camera_rotate:
            obj.image = pygame.transform.rotate(image, obj.degrees)
        
        obj.image = pygame.transform.scale_by(obj.image, zoom_factor)
    ...
    def addMap(self, base, full_map):
        ...
        big = [self.y - (HEIGHT // (2 * self.zoom)), self.y + (HEIGHT // (2 * self.zoom))]
        boi = [self.x - (WIDTH // (2 * self.zoom)), self.x + (WIDTH // (2 * self.zoom))]
        ...
        for row in range(0, MAP_RC[1], 1):
            for col in range(0, MAP_RC[0], 1):
                if full_map.transparent[row][col] != None: 
                    full_map.transparent[row][col].rect.center = [(col * self.zoom) - self.anchor[0] - self.gimble[0], (row * self.zoom) - self.anchor[1] - self.gimble[1]]
                    self.effects_sprite.add(full_map.transparent[row][col])

                img = full_map.tiles[row][col].image

                full_map.tiles[row][col].image = pygame.transform.scale(img, (self.zoom, self.zoom))
                full_map.tiles[row][col].rect = img.get_rect()
                
                full_map.tiles[row][col].rect.center = [(col * self.zoom) - self.anchor[0] - self.gimble[0], (row * self.zoom) - self.anchor[1] - self.gimble[1]]

                self.map_sprite.add(full_map.tiles[row][col])
    

    # IF WE EMPTY, THE CAMERA LAGS
